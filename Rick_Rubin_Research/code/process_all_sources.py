    ...
    with open('data/rick_rubin_source_analysis.csv', 'a', newline='') as f:
        ...
        for source in sources:
            ...
            if match:
                ...
                url = match.group(2)
                source_type = "Unknown"
                if "article" in title.lower() or "news" in title.lower() or "guide" in title.lower() or "history" in title.lower() or "insights" in title.lower() or "review" in title.lower() or "lessons" in title.lower() or "things" in title.lower() or "opinion" in title.lower() or "stripped down" in title.lower() or "creativity" in title.lower() or "excerpt" in title.lower():
                    source_type = "Article"
                    ...
                    source_type = "Book"
                elif "credits" in title.lower() or "discography" in title.lower():
                    source_type = "Database/Credits"
                elif "academic" in title.lower() or "scholarly" in title.lower() or ".edu" in url or ".ac." in url:
                    source_type = "Academic/Scholarly"
